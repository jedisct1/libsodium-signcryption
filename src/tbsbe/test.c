...
int main(void)
{
    ...
    unsigned char recipient_pk[crypto_signcrypt_tbsbe_PUBLICKEYBYTES];
    unsigned char recipient_sk[crypto_signcrypt_tbsbe_SECRETKEYBYTES];
    unsigned char crypt_key[crypto_signcrypt_tbsbe_SHAREDBYTES];
    ...
    crypto_signcrypt_tbsbe_keygen(sender_pk, sender_sk);
    crypto_signcrypt_tbsbe_keygen(recipient_pk, recipient_sk);
    randombytes_buf(nonce, sizeof nonce);

    /* sender-side */

    if (crypto_signcrypt_tbsbe_sign_before(st, crypt_key, (const unsigned char *) "sender",
        ...
        crypto_signcrypt_tbsbe_sign_after(st, sig, sender_sk, c, sizeof c) != 0) {
        return 1;
    }

    /* recipient-side */

    if (crypto_signcrypt_tbsbe_verify_before(
        ...
        crypto_signcrypt_tbsbe_verify_after(st, sig, sender_pk, c, sizeof c) != 0) {
        return 1;
    }

    /* the sender can also be publicly verified */

    if (crypto_signcrypt_tbsbe_verify_public(
